...
const Navbar = () => {
    ...
    return (
        <nav className="fixed top-0 right-[50%] translate-x-[50%] z-40 py-4 lg:pt-6 flex_between container_width bg-white">
            <Link href="/">
            ...
            <ul className={`font-heading transition-all absolute flex flex-col items-center gap-13 translate-x-full pt-24 z-30 bg-white ${openMenu && "!bg-gray-900 !translate-x-0"} h-screen w-screen top-0 left-0 lg:flex-row lg:gap-10 lg:relative lg:h-auto lg:w-auto lg:pt-0 lg:translate-x-0`}>
                {
                    nav_links.map((link, i) => (
                        <Link key={i} href="/" onClick={() => {!link.active && alert("This is just a demo"); setOpenMenu(false)}} className={`w-[fit-content] cursor-pointer pb-[1px] transition-all text-gray-300 lg:text-gray-500 hover:text-gray-800 text-[19px] lg:text-[17.5px] font-bold ${link.active && "border-b-3 border-amber-600 font-heading !text-gray-50 lg:!text-gray-950 !font-bold"}`}>{link.name}</Link>
                    ))
                }
            </ul>
